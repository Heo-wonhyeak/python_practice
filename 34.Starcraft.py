...
class Tank(AttackUnit):
    # 시즈모드 : 탱크를 지상에 공격시켜 데미지 증가, 이동 불가
    seize_developed = False # 시즈모드 연구 기본은 False
...
class FlyableAttackUnit(AttackUnit,Flyable):
    def __init__(self, name, hp, damage, flying_speed):
        AttackUnit.__init__(self, name, hp, damage, 0) # 공중유닛은 지상의 속도가 없기 때문에 0 값을 입력 
...
for unit in attack_units:
    if isinstance(unit, Marine): # 만들어진 객체가 어디의 인스턴스인지 확인하는 것 
        unit.stimpack()
